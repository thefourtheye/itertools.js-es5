...
describe('[Failure Cases] when compress is passed', function () {
  ...
  it('lesser than two arguments, it should throw an error', function () {
    expect(function () {
      return compress();
    }).to.throw(TypeError, /Required argument 'data' \(pos 1\) not found/);
    expect(function () {
      return compress([]);
    }).to.throw(TypeError, /Required argument 'selectors' \(pos 2\) not found/);
  });

  it('greater than two arguments, it should throw an error', function () {
    expect(function () {
      return compress([], [], []);
    }).to.throw(TypeError, /compress\(\) takes at most 2 arguments \(3 given\)/);
  });
});
...
describe('[Happy Cases] when compress is passed', function () {
  ...
  it('selectors and data of equal size and all selectors are truthy', function () {
    expect(Array.from(compress([1, 2, 3, 4, 5, 6, 7], [true, [0], 1, 3.14, ' ', {}, []]))).to.deep.equal([1, 2, 3, 4, 5, 6, 7]);
  });
});