...
describe('[Failure Cases] when dropwhile is passed', function () {
  ...
  it('no arguments, it should throw an error', function () {
    expect(function () {
      return dropwhile();
    }).to.throw(TypeError, 'dropwhile expected 2 arguments, got 0');
  });

  it('one argument, it should throw an error', function () {
    expect(function () {
      return dropwhile(1);
    }).to.throw(TypeError, 'dropwhile expected 2 arguments, got 1');
  });

  it('more than two arguments, it should throw an error', function () {
    expect(function () {
      return dropwhile(1, 2, 3);
    }).to.throw(TypeError, 'dropwhile expected 2 arguments, got 3');
  });
...
describe('[Happy Cases] when dropwhile is passed', function () {
  ...
  it('an empty iterable, it should return an empty iterable', function () {
    expect(Array.from(dropwhile(function () {
      return 1;
    }, []))).to.deep.equal([]);
  });
  ...
  it('a valid iterable with predicate failures should return rest of the items from the iterable', function () {
    expect(Array.from(dropwhile(function (x) {
      return x < 2;
    }, [1, 2, 3, 4]))).to.deep.equal([2, 3, 4]);
  });

  it('a valid iterable with alternating data should return proper result', function () {
    expect(Array.from(dropwhile(function (x) {
      return x < 7;
    }, [1, 3, 5, 7, 5, 3, 1]))).to.deep.equal([7, 5, 3, 1]);
  });
});